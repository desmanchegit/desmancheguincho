...
const TEST_DATA_ADMIN_EMAIL = "admin@centraldesmanches.com";
...
/**
 * Clears operational and demonstration records while keeping the single
 * production administrator account. System configuration and subscription
 * plan definitions are deliberately retained so the application remains
 * configured after the cleanup.
 */
export const clearTestData = sqlite.transaction((adminEmail: string): ClearTestDataResult => {
  if (adminEmail.trim().toLowerCase() !== TEST_DATA_ADMIN_EMAIL) {
    throw new Error("invalid_cleanup_admin");
  }

  const preservedAdmin = sqlite.prepare(`
    SELECT id
    FROM users
    WHERE lower(email) = ? AND type = 'admin'
  `).all(TEST_DATA_ADMIN_EMAIL) as Array<{ id: string }>;

  if (preservedAdmin.length !== 1) {
    throw new Error("preserved_admin_not_found");
  }

  // The order follows the foreign-key relationships, so this also works when
  // SQLite foreign-key enforcement is enabled by the hosting environment.
  const tablesToClear = [
    "asaas_webhook_events",
    "asaas_creation_intents",
    "billing_transactions",
    "desmanche_billing",
    "reviews",
    "chat_messages",
    "chat_rooms",
    "pre_proposal_messages",
    "pre_proposal_rooms",
    "proposals",
    "order_images",
    "negotiations",
  ...
  for (const table of tablesToClear) {
    deletedRecords += sqlite.prepare(`DELETE FROM ${table}`).run().changes;
  }
  deletedRecords += sqlite.prepare("DELETE FROM users WHERE id <> ?").run(preservedAdmin[0].id).changes;
